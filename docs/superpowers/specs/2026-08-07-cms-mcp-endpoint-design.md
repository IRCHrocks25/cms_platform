...
### 3.5 Audit — real model (revised 2026-08-08)

**Decision (owner, 2026-08-07):** a single `record_mcp_call(...)` in the `tools/call`
dispatcher chokepoint so no future tool can forget to log.

**Revision (owner, 2026-08-08):** CMS-9 lands the real `McpAuditLog` model and migration
(CMS-6 pulled into this PR), not a logger-only seam. Rows are written for **reads** —
deliberate: the team initially shares one superuser Claude connection, so knowing who
read which client's content is the only attribution available. Write tools remain out of
scope for CMS-9; they will reuse the same table later.

Bounds (required because read volume ≫ write volume):

- Exactly **one insert per `tools/call`** — never per-field or per-site fan-out
  (`list_sites` → one row with `tenant=NULL`).
- Arguments and field values are never stored.
- Insert failures are logged and **swallowed** — an audit outage must not take down a read.
...
| `dispatch.py` | JSON-RPC envelope, method table, `record_mcp_call` → `McpAuditLog` |
...
```python
# api/models.py — McpAuditLog(actor, tenant, action, performed_via="MCP", timestamp)
# api/mcp/dispatch.py
record_mcp_call(actor=auth.user, tenant=tenant_or_none, tool=name, performed_via="MCP")
return TOOLS[name](...)
```

`record_mcp_call` inserts one `McpAuditLog` row. The call site stays in the dispatcher
(CMS-6's original chokepoint requirement). Tenant is stamped only when the principal may
reach that site; `list_sites` and access denials leave `tenant` null.

Revised 2026-08-08 by the owner: CMS-9 ships the model + migration (CMS-6 absorbed).
Reads are logged on purpose. Failures in the insert path must not propagate to the client.
...
34. `makemigrations --check --dry-run` reports no changes after the committed `McpAuditLog` migration.
35. Every successful `tools/call` writes exactly one `McpAuditLog` row (`performed_via="MCP"`); `list_sites` does not fan out per site.
36. If the audit insert raises, the tool read still returns HTTP 200 with its normal result.
...
1. **Audit model landed in CMS-9** (§3.5 revised 2026-08-08) — CMS-6's model/migration
   scope was absorbed; close CMS-6 when this PR merges.
